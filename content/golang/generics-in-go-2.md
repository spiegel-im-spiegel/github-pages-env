...
update = "2018-11-11T12:22:12+09:00"
...
ならば，最初から汎化した型で記述してしまえば型ごとに同じコードを量産しなくてもいんじゃね？ という発想になる[^oaoo1]。
これが総称型の原点である。
...
たとえば Java で総称型は以下のような記述になる[^java1]。
...
[^sort1]: Slice のソートについてはバージョン 1.8 から任意の型に対応する [`sort`]`.Slice()` 関数が用意された。内部で [`reflect`] パッケージを使っているが，かなり巧妙に組まれているため，パフォーマンス低下は殆どないらしい。ただし slice 以外のインスタンスを指定すると（コンパイル時ではなく）実行時に panic を吐く。詳しくは「[ソートを使う]({{< ref "/golang/sort.md" >}})」を参照のこと。
...
また，現行版 [Go 言語]では総称型を用いずとも interface 型と [`reflect`] パッケージを使ってかなりの部分を代替できるが，コード量のコスト高を別にしても，記述の正しさは実行時での評価ではなくコンパイル時に評価してほしい。
...
継承を利用した制約と異なり型コントラクトの自由度は高く，したがって応用範囲も広い。
たとえばポインタ型を要求するなら
