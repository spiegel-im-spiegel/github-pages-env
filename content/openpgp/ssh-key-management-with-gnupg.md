...
tags = [ "openpgp", "openssh", "management", "cryptography", "gnupg" ]
pageType = "text"
...
いちばん簡単なのはコマンドラインで `--quick-add-key` を使う方法。
こんな感じに鍵指紋とアルゴリズム（または楕円曲線名）と機能と有効期限（`0` なら無期限）を指定する。

```text
$ gpg --quick-add-key 011C720B03D2E1D6BCFA98391DFF44901121B61D ed25519 auth 0
```

もうひとつは `--edit` コマンドを使う方法。
`--expert` オプションと一緒に使うと幸せになれる。
