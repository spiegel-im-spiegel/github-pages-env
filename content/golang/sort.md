...
`Len()`, `Less()`, `Swap()` 各メソッドを持つ [interface] を Sorter と呼び，以下のように定義されている。
...
つまり Sorter インタフェースを持つ型であれば [`sort`].`Sort()` 関数でソート可能ということになる。
たとえば以下のオブジェクト集合を考える。
...
`Planet` オブジェクトの集合に対する Sorter インタフェースはこんな感じにする[^str]。

[^str]: 今回は簡単のため [slice] を使っているが，データ集合は [slice] である必要はなく Sorter インタフェースを持つ任意のオブジェクトであればよい。
