...
3. `WeekdayJp` 型の値が `Sunday` 〜 `Saturday` の範囲外の場合， `StringJp()` および `ShortStringJp()` メソッドの出力は [`time`]`.Weekday.String()` メソッドと同じにしたい
...
LLM はじめとする最近流行りの生成 AI の機能とは「翻案の大量生産」であり，膨大な翻案空間から何をどうやって拾い上げるかについては，アルゴリズムの設計を含め（今のところ）人の側に委ねられている。
