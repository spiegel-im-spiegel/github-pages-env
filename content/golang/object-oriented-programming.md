...
update = "2016-11-17T15:28:56+09:00"
...
[^call]: [Go 言語]の関数呼び出しにおいて引数の渡し方は基本的に「値渡し」である。「参照渡し」にするにはポインタを値として渡せばよい。メソッド・レシーバについては，関数の呼び出し側インスタンスがポインタか否かに関係なく，値渡しの場合は値が，参照渡しの場合はポインタが渡される。詳しくは「[関数とポインタ]({{< relref "golang/function-and-pointer.md" >}})」を参照のこと。
