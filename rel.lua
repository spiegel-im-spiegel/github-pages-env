...
local path = hugo.pathjoin("content", arg[1])
if #arg > 1 then
	section = arg[1]
	if section == "remark" then
		if #arg > 2 then
			path = hugo.pathjoin("content", arg[1], arg[2], arg[3])
		else
			path = hugo.pathjoin("content", arg[1], os.date("%Y"), arg[2])
		end
	else
		path = hugo.pathjoin("content", arg[1], arg[2])
	end
end
