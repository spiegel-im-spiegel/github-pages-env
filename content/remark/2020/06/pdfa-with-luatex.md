...
PDF/A とは可搬性と永続性を考慮した PDF 構成で， ISO 19005 シリーズとして定義されている。
PDF/A の種別は大まかに
...
更に PDF/A-3 では任意のドキュメント・データの埋め込みが許容されているが，さすがにこれは可搬性や永続性の観点からは要らない機能と言えるだろう。
...
なお，メタデータを含む $\mathrm{Lua\LaTeX}$ テキストを処理するとメタデータの内容を `*.xmpdata` ファイルに吐き出すようだが，メタデータを変更したらこのファイルをいったん削除してから再処理する必要がある（上書きで変更しないみたい）。
また，実際に PDF ファイルに埋め込まれる XMP メタデータの内容は `pdfa.xmpi` に出力されるので参考になると思う。
