...
また，イテレータに変換する際に `Backward()` 関数で逆順に並び替えたり `Chunk()` 関数で `slice` を分割することもできる。

`Collect()` 関数を使ってイテレータを `slice` に変換することもできる。
前節の `randoms()` 関数であれば
...
のように書く。
また `AppendSeq()` 関数で既にある `slice` インスタンスにイテレータの出力を追加したり，`Sorted()` 関数等を使った並び替えもできる。
