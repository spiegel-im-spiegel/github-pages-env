...
色々試してみたが，どうも `String` 等のコレクションを含む構造体の `Copy` トレイトは実装できないようだ。
これは `derive` を使った実装が用意されていないためらしい[^cpy1]。

[^cpy1]: `derive` を使った `Copy` トレイトの実装はあくまでも bitwise なもので `String` 等でこの `Copy` を実装するとポインタ値がコピーされるだけなので意味がないそうな。
