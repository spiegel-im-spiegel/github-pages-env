...
## Go 1.25.5 の導入 【2025-12-20 更新】 {#update}
...
CVE ID ベースで2件の脆弱性修正がある（1.25.0 からの累積で13件）。

[Go]: https://go.dev/
{{< /div-box >}}

[Ubuntu] の APT で管理している [Go] コンパイラは古いので，[ダウンロードページ](https://go.dev/dl/ "Downloads - go.dev")からバイナリ（[`go1.25.5.linux-amd64.tar.gz`](https://go.dev/dl/go1.25.5.linux-amd64.tar.gz)）を取ってきてインストールすることを推奨する。
以下は完全手動での作業例。

```text
$ cd /usr/local/src
$ sudo curl -LO "https://go.dev/dl/go1.25.5.linux-amd64.tar.gz"
$ cd ..
$ sudo unlink go # 以前の Go が入っている場合
$ sudo tar xvf src/go1.25.5.linux-amd64.tar.gz
$ sudo mv go go1.25.5
$ sudo ln -s go1.25.5 go
...
```text
$ go install golang.org/dl/25.5@latest
$ go1.25.5 download
$ go1.25.5 version
go version go1.25.5 linux/amd64
```
