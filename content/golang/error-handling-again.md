...
関数 `foo()` と `bar()` から返却される error が同値であれば処理をまとめられる筈である。
...
と書き換えても駄目である。
[`xerrors`]`.New()` 関数はインスタンスへのポインタ値を返すが `foo()` や `bar()` 関数で返される error と `ErrInstance1` は異なるインスタンスなので [Go 言語]の同値演算子（equality operator）では（ポインタ値を比較するだけで）同値であることを示せない。
したがって実行結果も
...
ただし，このままでは [`xerrors`]`.Is()` 関数によるインスタンスの同値性を正しく検証できない。
...
これで同値演算子 `==` の代わりに `Num.Is()` および `wrapError.Is()` 関数がインスタンスの同値性をチェックしてくれる。
