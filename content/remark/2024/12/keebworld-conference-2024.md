...
そもそもメカニカルキーボード自体を使い出したのが[ごく最近]({{< ref "/remark/2024/11/mechanical-keyboard.md" >}} "はじめてのメカニカルキーボード")なんだよな。
...
[^kb2]: 特定用途というなら FPS (First-Person Shooter) とかでキーボード入力する人なら30%キーボードはむしろ需要があるかも？ ハードウェア要件が厳しいかもだけど。そういえば {{% keytop %}}`Shift`{{% /keytop %}} や {{% keytop %}}`Ctrl`{{% /keytop %}} といったコンビネーション・キーをフットペダルにアサインした製品を見かけたことがあるな。
...
日本語圏のエンジニアは Bluesky であまり見かけない印象を受ける。
真面目に探してないせいかも知れないが。
Mastodon は他サーバのユーザは探しにくいので動向が分からない。

今回はタイムテーブルをちゃんと見てなくて「日没までには帰ろう」とか呑気に考えてたら17時半過ぎまでみっちりだった。
...
- [はじめてのメカニカルキーボード]({{< ref "/remark/2024/11/mechanical-keyboard.md" >}})
