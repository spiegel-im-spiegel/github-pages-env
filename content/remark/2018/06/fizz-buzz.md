...
date =  "2018-06-01T10:19:29+09:00"
...
このルールをプログラムで実装するのが新人研修とかでよく行われているらしい。

というわけで，早速 [Go 言語]で書いてみる。
今回は素朴にこんな感じ。
...
完全に動くコード（の筈）なので動作確認は各自でどうぞ[^gcc1]。
ロートル・エンジニアから見ると auto 変数めっさ便利（笑）
でも if 文や for 文の式を括弧で括るのかったるいし，型を前置するの違和感が半端ない。

[^gcc1]: C++ コードの動作確認は [MinGW-w64 Windows版]({{< relref "remark/2018/03/mingw-w64.md" >}} "MinGW-w64 を導入する")の GCC 7.3 でコンパイルしたバイナリで行った。

これ書いててふと思ったが，私の[母国語]({{< relref "remark/2015/programming-language.md" >}})って既に [Go 言語]になってるな。
